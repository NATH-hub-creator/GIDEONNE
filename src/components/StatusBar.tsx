...
function StatusBar() {
  ...
  useEffect(() => {
    const verifier = async () => {
      try { setOllamaActif(await invoke<boolean>("verifier_ollama")); }
      catch { setOllamaActif(false); }
    };
    verifier();
    const intervalle = setInterval(verifier, 30_000);
  ...
  return (
    <footer className="status-bar" role="status">
      ...
      <div className="status-item">
        <span>{t("status.modele")} : </span><strong>{modeleActif || t("status.aucunModele")}</strong>
      </div>
      <div className="status-item">
        <span>{t("status.langue")} : </span><strong>{i18n.language.toUpperCase()}</strong>
      </div>
