...
function ChatWindow() {
  ...
  const gererToucheClavier = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      <div className="messages-liste" role="log" aria-live="polite">
        ...
        {chargement && (
          <div className="message message--assistant message--chargement">
            ...
            <div className="points-suspension"><span></span><span></span><span></span></div>
          </div>
        )}
        <div ref={finListeRef} />
      </div>
      <div className="saisie-zone">
        <VoiceButton onTranscription={(t) => setSaisie((p) => p + " " + t)} />
        <textarea
          ref={textareaRef}
          className="saisie-texte"
          value={saisie}
