// ============================================================
// commands/ai.rs — Commandes d'inférence LLM via Ollama
// Interface entre le frontend React et le serveur Ollama local
// ============================================================
...
#[derive(Debug, Serialize, Deserialize)]
struct RepListeModeles {
    models: Vec<ModeleOllama>,
}
...
pub async fn envoyer_message(requete: RequeteIA) -> ReponseIA {
    tracing::info!("Envoi message à Ollama — modèle: {}", requete.modele);
    match appel_ollama(requete).await {
        Ok(contenu) => ReponseIA {
            contenu,
            modele: "ollama".to_string(),
            succes: true,
            erreur: None,
        },
        Err(e) => {
            tracing::error!("Erreur Ollama : {}", e);
            ReponseIA {
                contenu: String::new(),
                modele: String::new(),
                succes: false,
                erreur: Some(format!("Erreur Ollama : {}", e)),
            }
        }
    }
}
...
async fn appel_ollama(requete: RequeteIA) -> Result<String> {
    ...
    let mut messages: Vec<serde_json::Value> = requete.historique.iter()
        .map(|m| serde_json::json!({"role": m.role, "content": m.contenu}))
        .collect();
