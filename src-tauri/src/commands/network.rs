...
/// Scanne les réseaux Wi-Fi disponibles (Linux via nmcli)
#[command]
    ...
    #[cfg(target_os = "linux")]
    {
            ...
            .output()
        {
            ...
            return sortie.lines().filter(|l| !l.is_empty()).map(|ligne| {
                let champs: Vec<&str> = ligne.splitn(4, ':').collect();
                ReseauWifi {
                    ssid: champs.first().unwrap_or(&"").to_string(),
                    signal: champs.get(1).and_then(|s| s.parse().ok()).unwrap_or(-100),
                    securite: champs.get(2).unwrap_or(&"Inconnu").to_string(),
                    connecte: champs.get(3).unwrap_or(&"non") == &"oui",
                }
            }).collect();
        }
    }
...
pub async fn obtenir_infos_reseau() -> Vec<InfoReseau> {
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(output) = Command::new("ip").args(["addr", "show"]).output() {
            ...
            let mut interface_courante: Option<String> = None;
            for ligne in sortie.lines() {
                if ligne.starts_with(|c: char| c.is_ascii_digit()) {
                    if let Some(nom) = ligne.split(':').nth(1) {
                        interface_courante = Some(nom.trim().to_string());
                    }
                    ...
                    if let Some(ref iface) = interface_courante {
                        interfaces.push(InfoReseau {
