...
/// Ecrit du contenu dans un fichier (crée ou écrase)
#[command]
...
pub async fn supprimer_element(chemin: String, recursif: bool) -> ResultatFs {
    ...
    let resultat = if path.is_dir() && recursif { fs::remove_dir_all(path) }
        else if path.is_dir() { fs::remove_dir(path) }
